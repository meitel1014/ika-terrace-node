...
- **Dashboard** ([src/browser/dashboard/](src/browser/dashboard/)) — 運営用の操作パネル（4 Workspace: Battle / 判定結果 / 設定 / caster）
...
| `confirmMatchCandidate` | `{ mode, candidateIndex }` | OCR 候補を確定して `matches` に追加・`records.csv`/GAS に統合送信 |
| `dismissMatchCandidate` | `{ mode, candidateIndex }` | OCR 候補を破棄 |
| `updateMatchCandidate` | `{ mode, candidateIndex, side, position, patch }` | 候補のプレイヤー名・ブキを手動修正 |
...
│   │       ├── under/               # 配信画面下部（ナワバリ/エリア共通）
│   │       ├── side/                # 会場左右分割（ナワバリ/エリア共通）
